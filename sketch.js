...
class squareSection {
  constructor(xpos, ypos, rotation) {
    ...
    this.diagonalLength = getHype(squareSection.prototype.size/2, squareSection.prototype.size/2);
    //Sets up lines in square and makes a copy that will be changed when running
    this.orginalLines = this.createLines(squareSection.prototype.linePerSide, squareSection.prototype.size);
    this.changedLines = JSON.parse(JSON.stringify(this.orginalLines));
  }
  draw() {
  ...
  createLines() {
    ...
    for (var i = (squareSize/linesPerSide)/2; i < squareSize; i += (squareSize/linesPerSide)) {
      let linePos = {};
      linePos.start = {x: -wallPosition, y:i - (squareSize/2)};
      let ranX = random(1, 3) * (i/3);
      //After starigth line with length is made on left side it is rotated 45 degrees
    ...
    for (var i = (squareSize/linesPerSide)/2; i < squareSize; i += (squareSize/linesPerSide)) {
      ...
      //After starigth line with length is made on bottom it is rotated 45 degrees
      linePos.end = rotatePoint(
        linePos.start.x, 
        linePos.start.y, 
        linePos.start.x, 
...
function setup() {
  ...
  squareSection.prototype.diagonalLength = getHype(squareSection.prototype.size/2, squareSection.prototype.size/2);
...
function draw() {
  ...
  for (var i = 0; i < squaresToDraw.length; i += 1){
    ...
    if (mouseEffect.checked()){
      ...
      let distanceMouse = ((dist(mouseX, mouseY, squaresToDraw[i].xpos, squaresToDraw[i].ypos))/getHype(width, height));
      for (var j = 0; j < squaresToDraw[i].orginalLines.length; j++){
        let line = squaresToDraw[i].orginalLines[j]
        squaresToDraw[i].changedLines[j].end = changeLine(line.start.x, line.start.y, line.end.x, line.end.y, distanceMouse);
      }
    }
...
function makeRow(y, start, angelWidth) {
  ...
  for (var i = angelWidth; i < width + angelWidth; i += angelWidth*2){
    //Sets angle
    let angle = (!((round(((i-angelWidth)/(angelWidth*2))+start))%2)) ? -45 : -225;
...
function changeLine(posX, posY, posXX, posYY, distance) {
  ...
  distance = invertEffect.checked() ? exponential(1 - distance) : logarithmic(distance);
  distance = distance > 1 ? 1 : distance;
...
function sizeLinesChanged(){
  //Runs when GUI is used to change size and lines perside
  //Makes sure values are entered
  if (Number.isInteger(int(sizeInput.value())) && Number.isInteger(int(linesInput.value()))){
    //Changes values
